<script lang="ts">
  ...
  function selectEpisode(ep: { locked: boolean; slug: string }) {
    if (ep.locked) return;
  ...
  function extractShowSlugFromEpisodeHtml(html: string): string | null {
    ...
    const a = doc.querySelector('a[href^="/ro/full-episodes/"], a[href^="/full-episodes/"]');
    if (!a) return null;
    const href = a.getAttribute("href")!;
    return norm(
      href.replace(/^\/ro\/full-episodes\//, "").replace(/^\/full-episodes\//, "").trim()
    );
  }

  function getMaxPageFromHtml(html: string): number {
    const doc = new DOMParser().parseFromString(html, "text/html");
    const pageNums = Array.from(doc.querySelectorAll('a[href*="/full-episodes/"]'))
  ...
  function parseEpisodesFromFullEpisodesHtml(html: string, currentEpisodeSlug: string) {
    const doc = new DOMParser().parseFromString(html, "text/html");

    let links = Array.from(doc.querySelectorAll('a.cover-bg[href^="/ro/episodes/"]'));
    if (!links.length) links = Array.from(doc.querySelectorAll('a[href^="/ro/episodes/"]'));
    ...
    return links.map((a) => {
      ...
      const number =
        lower.startsWith("trailer-") ? 0 : Number(slug.match(/episode-(\d+)/i)?.[1] ?? NaN);
  ...
  async function fetchAllEpisodes(showSlug: string, currentEpisodeSlug: string) {
    ...
    if (maxPage > 1) {
      for (let p = 2; p <= maxPage; p++) {
        ...
        const r = await fetch(`${baseRO}/${p}`, { headers: { accept: "text/html" } });
        if (r.status === 404) break;
        if (!r.ok) continue;
        all = all.concat(parseEpisodesFromFullEpisodesHtml(await r.text(), currentEpisodeSlug));
      }
    }
  ...
  function getNextEpisodeSlug(): string | null {
    ...
    for (let i = idx + 1; i < sorted.length; i++) {
      const e = sorted[i];
      if (!e.locked && norm(e.slug) !== cur) return e.slug;
    }
    return null;
  }
  ...
  $: if (videoEl) {
    ...
    const onTimeUpdate = () => { if (shouldAutoAdvance(v)) goNextEpisode(); };
  ...
  onMount(async () => {
    ...
    pageProgress = "1/1";
    try {
      episodes = await fetchAllEpisodes(showSlug, data.slug);
    } finally {
  ...
  <div class="player-right">
    ...
    {#if loadingEpisodes}
      <div class="episodes-loading">
        Se încarcă episoadele... {#if pageProgress}<span>{pageProgress}</span>{/if}
      </div>
    {/if}
    ...
    {#if episodes?.length}
      <div class="episodes">
        {#each episodes as ep (ep.slug)}
          <button
            class="ep {ep.active ? 'active' : ''} {ep.locked ? 'locked' : ''}"
            on:click={() => selectEpisode(ep)}
            disabled={ep.locked}
          >
            {ep.number === 0 ? "Trailer" : ep.number}
          </button>
        {/each}
      </div>
    {/if}
